...
import { LpaDocument, DocumentType, Ticker } from "~/types/document";
...
export const useDocumentSelector = () => {
  const [availableDocuments, setAvailableDocuments] = useState<LpaDocument[]>(
    []
  );
  ...
  const selectTicker = (ticker: Ticker) => {
    console.log("setting selected ticker", ticker)
